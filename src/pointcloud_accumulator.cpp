...
class PointCloudAccumulator : public rclcpp::Node {
 public:
  PointCloudAccumulator() : Node("point_cloud_filter_node") {
    ...
    point_cloud_subscriber_1_ =
        this->create_subscription<sensor_msgs::msg::PointCloud2>(
            input_topic_1_, 1,
            std::bind(&PointCloudAccumulator::callback1, this,
                      std::placeholders::_1));

    point_cloud_subscriber_2_ =
        this->create_subscription<sensor_msgs::msg::PointCloud2>(
            input_topic_2_, 1,
            std::bind(&PointCloudAccumulator::callback2, this,
                      std::placeholders::_1));

    // service for storing the pointcloud
    point_cloud_dump_service_ = this->create_service<std_srvs::srv::Empty>(
        "point_cloud_dump",
        std::bind(&PointCloudAccumulator::pointCloudDumpCallback, this,
                  std::placeholders::_1, std::placeholders::_2));
    // Advertise the filtered point cloud topic
    total_cloud_publisher_ =
    ...
    RCLCPP_INFO(this->get_logger(), "input_topic_1: %s",
                input_topic_1_.c_str());
    RCLCPP_INFO(this->get_logger(), "input_topic_2: %s",
                input_topic_2_.c_str());
    RCLCPP_INFO(this->get_logger(), "output_topic: %s", output_topic_.c_str());
    RCLCPP_INFO(this->get_logger(), "total_cloud_frame: %s",
                total_cloud_frame_.c_str());
 ...
 private:
  ...
  void pointCloudDumpCallback(
    ...
    RCLCPP_INFO(this->get_logger(),
                "\033[96m \033[1m Point cloud has been dumped to "
                "total_cloud.ply \033[0m");
  }
  ...
  void filterPointCloud() {
    // remove outliers
    Eigen::Vector3d pt;
    for (int i = 0; i < total_cloud_->size(); i++) {
      pt << total_cloud_->points[i].x, total_cloud_->points[i].y,
          total_cloud_->points[i].z;
      if (pt.norm() > 0.30) {
        total_cloud_->points.erase(total_cloud_->points.begin() + i);
      }
    }
    // Apply PassThrough filter
    ...
    voxel_filter_.setLeafSize(voxel_grid_size, voxel_grid_size,
                              voxel_grid_size);
